...
class Rotation(Node):
    def __init__(self):
        ...
        self.create_subscription(Int8, "control_system/rotation",
                                 self.turn_designate_angular, qos_profile_sensor_data)

        # for passplanning
        self.pub = self.create_publisher(
            Twist, "cmd_vel", qos_profile_sensor_data)
        self.create_subscription(
            Odometry, "odom", self.now_angular, qos_profile_sensor_data)
        self.angular = None
    ...
    def turn_designate_angular(self, massage):
        ...
        # first half
        _vel_sum, _vel_f = self.set_velocity(
            _sign, _velocity, _vel_sum, _total_angular)

        _velocity = _vel_f[_total_angular]

        # second half
        _vel_sum, _vel_s = self.set_velocity(-_sign,
                                             _velocity, _vel_sum, _total_angular)
    ...
    def set_velocity(sign, velocity, vel_sum, total_angular):
        ...
        for i in range(1, total_angular+1):
            __acceleration = sign * \
                math.sin(math.radians(i*4*math.pi/total_angular))+sign
